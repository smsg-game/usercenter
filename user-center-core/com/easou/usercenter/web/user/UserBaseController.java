...
public class UserBaseController extends BaseController {
	...
	private final static String DEFAULT_BACK = "userCenter";
	...
	protected ArrestValidation checkLogin(final HttpServletRequest request, ModelMap model) {
		...
		eucToken.setToken(ticketGrantingTicketId);
		Long userId = ServiceFactory.getInstance().getAuthVerifyService().verify(eucToken);
		if (null == userId || 0 == userId.longValue()) {
			removeLoginAssert(request);
			ArrestValidation av = new ArrestValidation(false);
			
			// 回调地址
			av.setBackUrl(request.getContextPath() + "/logout");
			return av;
		}
