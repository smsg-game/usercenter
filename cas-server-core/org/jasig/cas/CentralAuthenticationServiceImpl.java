...
public final class CentralAuthenticationServiceImpl implements CentralAuthenticationService {
    ...
    private final Log log = LogFactory.getLog(ServiceValidateController.class);
    ...
    public Assertion validateServiceTicket(final String serviceTicketId, final Service service) throws TicketException {
        ...
        if (serviceTicket == null) {
            log.error("ServiceTicket [" + serviceTicketId + "] does not exist.");
            throw new InvalidTicketException();
        }
        ...
        try {
            synchronized (serviceTicket) {
                if (serviceTicket.isExpired()) {
                    log.error("ServiceTicket [" + serviceTicketId + "] has expired.");
                    throw new InvalidTicketException();
                }
            ...
            //SDK改造时，增加serviceTicket.getGrantingTicket().getId()参数
            return new ImmutableAssertionImpl(authentications, serviceTicket.getService(),serviceTicket.getGrantingTicket().getId(), serviceTicket.isFromNewLogin());
        } finally {
            if (serviceTicket.isExpired()) {
    ...
    public String createTicketGrantingTicket(final Credentials credentials) throws TicketCreationException {
        ...
        try {
            ...
            /* TODO Seek 2013-10-12  start */
            UsernamePasswordCredentials aCredentials = null;
            if(credentials instanceof UsernamePasswordCredentials) {
            	aCredentials = (UsernamePasswordCredentials)credentials;
            	
            	ticketGrantingTicket = new TicketGrantingTicketImpl(
                		TokenUtils.getTokenByUserInfo(aCredentials.getUsername(), aCredentials.getPassword(), false),
                      authentication, this.ticketGrantingTicketExpirationPolicy);
            }/* TODO Seek 2013-10-12  end! */
            else {
            	ticketGrantingTicket = new TicketGrantingTicketImpl(
                        this.ticketGrantingTicketUniqueTicketIdGenerator
                            .getNewTicketId(TicketGrantingTicket.PREFIX),
