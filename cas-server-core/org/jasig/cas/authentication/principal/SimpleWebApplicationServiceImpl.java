...
public final class SimpleWebApplicationServiceImpl extends
    ...
    public static SimpleWebApplicationServiceImpl createServiceFrom(
        ...
        if (!StringUtils.hasText(serviceToUse)) {
        	...
        	if(serviceToUse.indexOf("qn=")==-1) {
        		// 通过用户中心登录,如果请求链接中有qn，将qn带给service应用
        		final String qn = request.getParameter(Constant.QN_TAG);
        		if(!StringUtil.isEmpty(qn)) {
        			if(serviceToUse.indexOf("?")==-1) {
        				serviceToUse=serviceToUse + "?" + Constant.QN_TAG + "=" + qn;
        			} else {
        				serviceToUse=serviceToUse + "&" + Constant.QN_TAG + "=" + qn;
        			}
        		}
