...
public abstract class AbstractPersonDirectoryCredentialsToPrincipalResolver
    ...
    public final Principal resolvePrincipal(final Credentials credentials,AuthenticationResult auth) {
        if (log.isDebugEnabled()) {
            log.debug("Attempting to resolve a principal...");
        }

        final String principalId = extractPrincipalId(credentials,auth);
     ...
     */
    protected abstract String extractPrincipalId(Credentials credentials,AuthenticationResult auth);
