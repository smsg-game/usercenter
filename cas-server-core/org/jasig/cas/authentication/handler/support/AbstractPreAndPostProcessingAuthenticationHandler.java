...
public abstract class AbstractPreAndPostProcessingAuthenticationHandler
	 ...
	 * 对用户信息进行鉴权
	 */
	public final AuthenticationResult authenticate(final Credentials credentials)
			throws AuthenticationException {
		if (!preAuthenticate(credentials)) {
			return new AuthenticationResult(false, null);
		}
		AuthenticationResult result = doAuthentication(credentials);
		final boolean authenticated = result.isSuccess();
		if (!authenticated)
			return new AuthenticationResult(postAuthenticate(credentials,
					authenticated), null);
		else {
			return result;
		}
